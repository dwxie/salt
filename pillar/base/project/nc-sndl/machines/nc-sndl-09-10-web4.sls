  ...
  zone:
    nginx:
      vhosts:
        ...
        jxsr-keeper.antelopecloud.cn.conf:
          server_names: [jxsr-keeper.antelopecloud.cn]
index_web:
  config: {INDEX_WEB_BROKER_URL: 'redis://:123456@127.0.0.1:6800/3', INDEX_WEB_CEPH_CONFIG: '',
    INDEX_WEB_CEPH_TS_POOL: event_storage, INDEX_WEB_IP: 218.95.36.34, INDEX_WEB_REDIS_DB: '2',
...
tracker:
  config:
    tracker:
      http_center_web: {local_ip: 218.95.36.34}
      max_peer_count: 32768
      max_relay_count: 256
      rabbitmq_message:
        exchange: tracker_msg
        queue_name: tracker_218.95.36.34_q
        routing_key: tracker_218.95.36.34
        url: ['amqp://topvdn:W1rvtlViBO3RLwklImKO@192.168.2.34:5672']
        vhost: /topvdn
