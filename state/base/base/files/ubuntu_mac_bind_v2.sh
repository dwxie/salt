...
generate_interfaces() {
    local ips="$(cat "${interfaces_file}_backup_for_bond" | grep -w "^address" |awk '{print $2}')"
    local ip_count="$(cat "${interfaces_file}_backup_for_bond" | grep -w "^address" | awk '{print $2}' | wc -l)"
    if [ "${ip_count}" -gt 0 ]
    then
...
EOF
        ...
        for ip in $ips
        do
            nic="$(cat "${interfaces_file}_backup_for_bond" | grep "${ip}" -B 2 | grep -v "bond" | grep -w "iface" | awk '{print $2}')"
            netmask="$(cat "${interfaces_file}_backup_for_bond" | grep "${ip}" -A 3 | grep -w "netmask" | awk '{print $2}')"
            gateway="$(cat "${interfaces_file}_backup_for_bond" | grep "${ip}" -A 3 | grep -w "gateway" | awk '{print $2}')"
            cat >> "${interfaces_file}" <<EOF
auto eth${nic_seq}
