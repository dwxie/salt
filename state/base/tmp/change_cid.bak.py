    ...
    if is_online():
        if is_pushing():
            ...
            if is_exist_in_conf():
                ...
                print " the camera dose not in the bond_list\n now do the add..."
                add_bondinfo()
    else:
...
def add_bondinfo():
    ...
    if carmer_type == 'D0':
        conf.set('FORCED_BOND_RELAY','"bond_info"+str(count_old)',str(cid[0])+'_208_'+relay_ip+':'+relay_port)
    elif carmer_type == 'C0':
        conf.set('FORCED_BOND_RELAY','"bond_info"+str(count_old)',str(cid[0])+'_192_'+relay_ip+':'+relay_port)
    elif carmer_type == 'E0':
        conf.set('FORCED_BOND_RELAY','"bond_info"+str(count_old)',str(cid[0])+'_224_'+relay_ip+':'+relay_port)
...
def change_to_CMCC():
    print 'now change to CMCC relay: %s' %
    carmer_type = get_role_result()
    bond_key = get_cam_bondkey_in_confile()
